...
def downloadVideo(link):
    ...
    try:
        ...
        print("You can install it using: winget install ffmpeg")
        print("The raw video and audio files are saved in the '{}' folder.".format(output_folder))
        return

    cmd = 'ffmpeg -hide_banner -loglevel error -y -i "{}" -i "{}" -c copy "{}"'.format(
    ...
    if subprocess.call(cmd, shell=True) == 0:
        if os.path.exists(combined_file_path):
            ...
            final_path = os.path.join(output_folder, '{}.mp4'.format(video_id))
            if os.path.exists(final_path):
                os.remove(final_path)
            os.rename(combined_file_path, final_path)
            print("Successfully merged and saved as {}.mp4".format(video_id))
        else:
            print("Error: Merged file was not created despite ffmpeg returning success.")
    else:
